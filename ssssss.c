...
int main() {
	...
	if (search(root, key) == NULL) {
		if (key % 10 == 0 || key % 10 == 1 || key % 10 == 3 || key % 10 == 6 || key % 10 == 7 || key % 10 == 8) printf("%d은 트리에서 찾을 수 없습니다.\n", key);
		else printf("%d는 트리에서 찾을 수 없습니다.\n", key);
	}
	else if (search(root, key)) {
		if (key % 10 == 0 || key % 10 == 1 || key % 10 == 3 || key % 10 == 6 || key % 10 == 7 || key % 10 == 8) printf("%d은 트리에서 찾을 수 있습니다.\n", key);
		else printf("%d는 트리에서 찾을 수 있습니다.\n", key);
	}*/
	/*root = insert(root, 50);
	insert(root, 18);
	insert(root, 64);
